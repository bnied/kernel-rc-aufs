...
#
# Additional option for toracat-friendly, one-off, kernel-rc-aufs building.
# Only build the base kernel-rc-aufs (--with baseonly):
%define with_baseonly     %{?_with_baseonly:        1} %{?!_with_baseonly:        0}

%global KVERREL %{pkg_version}-%{pkg_release}.%{_target_cpu}

# If requested, only build base kernel-rc-aufs package.
%if %{with_baseonly}
%define with_doc 0
%define with_perf 0
%define with_tools 0
...
#
# List the packages required for the kernel-rc-aufs build.
#
...
Source2: config-%{LKAver}.%{LKRCver}-x86_64
...
#
# This macro does requires, provides, conflicts, obsoletes for a kernel-rc-aufs package.
...
#
# This macro creates a kernel-rc-aufs-<subpackage>-devel package.
...
Provides: installonlypkg(kernel)\
Provides: installonlypkg(kernel-rc-aufs)\
AutoReqProv: no\
...
#
# This macro creates an empty kernel-rc-aufs-<subpackage>-devel-matched package that
...
#
# This macro creates a kernel-rc-aufs-<subpackage>-modules-extra package.
...
Provides: installonlypkg(kernel-rc-aufs-module)\
...
#
# This macro creates a kernel-rc-aufs-<subpackage>-modules package.
...
#
# this macro creates a kernel-rc-aufs-<subpackage> meta package.
...
#
# This macro creates a kernel-rc-aufs-<subpackage> and its -devel.
#   %%define variant_summary The Linux kernel-rc-aufs compiled for <configuration>
...
%{__mv} linux-%{LKAver}-%{LKRCver} linux-%{KVERREL}
...
# Files for 'make scripts' to succeed with kernel-rc-aufs-devel.
...
# Files for 'make prepare' to succeed with kernel-rc-aufs-devel.
...
%ifarch x86_64
# Files required for 'make prepare' to succeed with kernel-rc-aufs-devel.
...
# Identify modules in the kernel-rc-aufs-modules-extras package
%{SOURCE20} $RPM_BUILD_ROOT lib/modules/%{KVERREL} %{SOURCE26}

#
# Generate the kernel-rc-aufs-core and kernel-rc-aufs-modules file lists.
#
...
# Don't include anything going into kernel-rc-aufs-modules-extra in the file lists.
xargs rm -fr < mod-extra.list

# Find all the module files and filter them out into the core and modules lists.
# This actually removes anything going into kernel-rc-aufs-modules from the directory.
...
# We use this for the directory lists in kernel-rc-aufs-core.
...
# for when the kernel-rc-aufs-devel package is installed.
ln -sf /usr/src/kernels/%{KVERREL} $RPM_BUILD_ROOT/lib/modules/%{KVERREL}/build

# Move the generated vmlinux.h file into the kernel-rc-aufs-devel directory structure.
if [ -f tools/bpf/bpftool/vmlinux.h ]; then
    mv tools/bpf/bpftool/vmlinux.h $RPM_BUILD_ROOT/usr/src/kernels/%{KVERREL}/
fi

# Purge the kernel-rc-aufs-devel tree of leftover junk.
...
# kernel-rc-aufs headers_install will remove any header files in /usr/include that
# it doesn't install itself.

%if %{with_headers}
# Install kernel-rc-aufs headers
...
#
# This macro defines a %%post script for a kernel-rc-aufs*-devel package.
...
#
# This macro defines a %%post script for a kernel-rc-aufs*-modules-extra package.
...
#
# This macro defines a %%post script for a kernel-rc-aufs*-modules package.
# It also defines a %%postun script that does the same thing.
...
# This macro defines a %%posttrans script for a kernel-rc-aufs package.
...
#
# This macro defines a %%post script for a kernel-rc-aufs package and its devel package.
...
#
# This macro defines a %%preun script for a kernel-rc-aufs package.
...
#
# This macro defines the %%files sections for a kernel-rc-aufs package
...
- The fourth release candidate of a kernel-rc-aufs package set for el9.
...
- The third release candidate of a kernel-rc-aufs package set for el9.

* Tue Sep 20 2022 Alan Bartlett <ajb@elrepo.org> - 5.19.10-0.rc2
- [https://www.kernel.org/pub/linux/kernel/v5.x/ChangeLog-5.19.10]
- The second release candidate of a kernel-rc-aufs package set for el9.

* Sun Sep 18 2022 Alan Bartlett <ajb@elrepo.org> - 5.19.10-0.rc1
- Updated with the 5.19.10 source tarball.
- [https://www.kernel.org/pub/linux/kernel/v5.x/ChangeLog-5.19.10]
- The first release candidate of a kernel-rc-aufs package set for el9.

* Sat Aug 13 2022 Alan Bartlett <ajb@elrepo.org>
- Forked this specification file to create a kernel-rc-aufs package set for the ELRepo Project.
