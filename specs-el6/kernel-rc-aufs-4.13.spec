...
# Build only the 32-bit kernel-rc-aufs-headers package.
%ifarch i386
%define with_std 0
%define with_nonpae 0
%define with_doc 0
...
# Build only the 32-bit kernel-rc-aufs packages.
%ifarch i686
%define with_doc 0
%define with_headers 0
%define with_firmware 0
...
# Build only the 64-bit kernel-rc-aufs-headers & kernel-rc-aufs packages.
%ifarch x86_64
%define with_nonpae 0
%define with_doc 0
%define with_firmware 0
...
if [ `uname -i` == "i386" ] && [ -f /etc/sysconfig/kernel ]; then
    /bin/sed -r -i -e 's/^DEFAULTKERNEL=kernel-rc-aufs-NONPAE$/DEFAULTKERNEL=kernel-rc-aufs/' /etc/sysconfig/kernel || exit $?
fi
...
%post NONPAE
if [ `uname -i` == "i386" ] && [ -f /etc/sysconfig/kernel ]; then
    /bin/sed -r -i -e 's/^DEFAULTKERNEL=kernel-rc-aufs$/DEFAULTKERNEL=kernel-rc-aufs-NONPAE/' /etc/sysconfig/kernel || exit $?
fi
